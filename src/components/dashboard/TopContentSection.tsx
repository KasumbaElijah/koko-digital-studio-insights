...
export const TopContentSection: React.FC<TopContentSectionProps> = ({ clientName, posts }) => {
  ...
  if (!topPosts || topPosts.length === 0) {
    return (
      <div className="w-full bg-white border border-gray-200 rounded-2xl p-8 shadow-sm mb-6 text-center">
        <h2 className="text-xl sm:text-2xl font-bold tracking-widest text-gray-900 uppercase mb-3 text-center font-heading">
          TOP PERFORMING CONTENT {clientName}
        </h2>
        <div className="max-w-md mx-auto py-8 text-gray-500 flex flex-col items-center">
          <div className="w-12 h-12 rounded-2xl bg-gray-100 flex items-center justify-center mb-3">
            <AlertCircle className="w-6 h-6 text-gray-400" />
          </div>
          <p className="text-sm font-bold text-gray-800">No Live Media Posts Synced Yet</p>
          <p className="text-xs text-gray-400 mt-1">
            Connect client Instagram Business or TikTok account in Settings and click Live Sync to stream top performing videos.
          </p>
        </div>
      </div>
    );
